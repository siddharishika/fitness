...
function ShowRecipe() {
    ...
    async function getRecipe() {
      ...
      try {
        let res = await axios.get(
          `${API_BASE_URL}/showrecipe/${initialRecipe._id}`,
          {
  ...
  useEffect(() => {
    async function checkLiked() {
      try {
        let likedRes = await axios.get(`${API_BASE_URL}/getlikedrecipes`, {
          withCredentials: true,
        });
        if (
          likedRes.data &&
          likedRes.data.data &&
          Array.isArray(likedRes.data.data.likedRecipes)
        ) {
          const exists = likedRes.data.data.likedRecipes.some(
            (r) => String(r._id) === String(recipe._id),
          );
          setLiked(exists);
        }
      } catch (err) {}
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      let res = await axios.post(
        `${API_BASE_URL}/recipe/addreview/${recipe._id}`,
        { review: reviewText },
        {
          withCredentials: true,
        },
      ...
      let updated = await axios.get(
        `${API_BASE_URL}/showrecipe/${recipe._id}`,
        {
          withCredentials: true,
        },
      );
          ...
          <Card.Body>
            ...
            <ul style={{ color: "#f4f4f8", paddingLeft: "1rem" }}>
              {recipe?.ingredients?.map((ingr, i) => (
                <li key={i}>
                  {ingr.ingredient} — {ingr.amount}grams
                </li>
              ))}
            </ul>
            <Card.Text style={{ color: "#f4f4f8" }}>
