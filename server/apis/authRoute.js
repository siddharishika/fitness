...
router.patch('/edituser', isLoggedIn, upload.fields([
  { name: 'profilePicture', maxCount: 1 },
]), async (req, res) => {
  try {
    ...
    if (!username || !email || !gender) {
      return res.status(400).json({ msg: "Username, email, and gender are required." });
    }
    ...
    if (password || passwordc) {
      if (!password || !passwordc) {
        return res.status(400).json({ msg: "Please enter and confirm your new password." });
      }
      if (password !== passwordc) {
