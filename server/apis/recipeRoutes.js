...
router.post('/addrecipe',isLoggedIn, isCoach, async(req,res)=>{
    ...
    try{
       ...
       // normalize process: allow array or newline/semicolon/comma separated string
       let processArr = [];
       if (Array.isArray(process)) processArr = process.map(p => String(p).trim()).filter(Boolean);
       else if (typeof process === 'string') processArr = process.split(/\r?\n|;|,/).map(p => p.trim()).filter(Boolean);

       // normalize ingredients: ensure array of { ingredient, amount }
       let ingredientsArr = [];
       ...
       const newRecipe = await Recipe.create({
        ...
        if (e.name === 'ValidationError' && e.errors) {
          response.validation = Object.keys(e.errors).map(k => ({ field: k, message: e.errors[k].message }));
...
router.get('/allrecipes',async(req,res)=>{
    ...
    try{
        ...
        let recipes=await Recipe.find({}).populate('user').populate('reviews.user');
...
router.get('/getmyrecipes',isLoggedIn, async(req,res)=>{
    try {
        let user=req.user;
        let result=await Recipe.find({user:user._id}).populate('reviews.user').exec();
...
router.patch('/recipe/addrating/:id', isLoggedIn, isNotRecipeOwner,  async (req, res) => {
    try {
...
router.post('/recipe/addreview/:id', isLoggedIn, isNotRecipeOwner, async (req, res) => {
    try {
        const userId = req.user._id;
        const id = req.params.id;
        let { review } = req.body;
