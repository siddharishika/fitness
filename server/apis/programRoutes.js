...
async function getCoverUrlFromSchedule(schedule = []) {
  ...
  const video = await FitnessVideo.findById(videoId)
    .select("imgFileUrl")
    .lean();
...
async function withProgramCoverImages(programs) {
  ...
  const videos = videoIds.length
    ? await FitnessVideo.find({ _id: { $in: videoIds } })
        .select("imgFileUrl")
        .lean()
    : [];
  ...
  return list.map((program) => {
    ...
    const scheduleImage = firstVideoId ? videoMap[String(firstVideoId)] : null;
    return {
      ...program,
...
router.post("/addprogram", isLoggedIn, isCoach, async (req, res) => {
  try {
    ...
    if (numDays < 1)
      return res.status(400).json({ msg: "numberOfDays must be >= 1" });
    ...
    await Program.create({
      ...
      description,
      timePerDay,
      file: fileUrl || PROGRAM_PLACEHOLDER,
    });
...
router.get("/allprograms/:tag", async (req, res) => {
  try {
    ...
    let programs = await Program.find({ tags: tag })
      .populate("coach")
      .populate("reviews.user");
    const data = await withProgramCoverImages(programs);
    ...
    try {
      ...
      for (let i = 0; i < found.rating.length; i++) {
        if (found.rating[i].user.toString() === userId.toString()) {
          return res
            .status(400)
            .json({ msg: "You have already rated this program" });
        }
      }
