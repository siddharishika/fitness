...
router.get("/show/:data", async (req, res) => {
  try {
    ...
    let fitnessVideos = await FitnessVideo.findById(data)
      .populate("coach")
...
router.patch(
  ...
  isCoach,
  isVideoAuthor,
  upload.fields([{ name: "imgFile", maxCount: 1 }]),
  async (req, res) => {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      let arr = tags.split(",");
      let found = await FitnessVideo.findById(id);
      let { fileId, imgFileId, filePath, imgFilePath, fileUrl, imgFileUrl } =
        found;
      await FitnessVideo.findByIdAndUpdate(id, {
        name,
        tags: arr,
  ...
  try {
    ...
    if (flag) {
      for (let i = 0; i < user.likedVideos.length; i++) {
        if (
          user.likedVideos[i] &&
          vid._id.toString() != user.likedVideos[i].toString()
        ) {
...
router.get("/allvideos/:tag", async (req, res) => {
  try {
    ...
    let fitnessVideos = await FitnessVideo.find({ tags: tag }).populate(
      "coach",
    );
    ...
    try {
      ...
      for (let i = 0; i < found.rating.length; i++) {
        if (found.rating[i].user.toString() === user._id.toString()) {
          return res
            .status(400)
            .json({ msg: "You have already rated this video" });
        }
      }
